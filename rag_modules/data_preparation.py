...
class DataPreparationModule:
    ...
    def load_documents(self) -> List[Document]:
        ...
        # # 不用Path函数路径生成繁琐且容易出错
        # 例如：file_path = "data" + "/" + "recipes" + "/" + "番茄炒蛋.txt"
        # 但是在这里是因为转换成 Path 对象后（data_path_obj），
        # 这个对象就像一个 “智能路径工具”，自带了各种实用功能
        for md_file in data_path_obj.rglob("*.md"):
            # 递归遍历data_path_obj代表的文件夹及其所有子文件夹
            # 找出所有后缀为.md的 Markdown 文件。
            # 读取文件内容，保持Markdown格式
            try:
                ...
                # 这个被下面的代码替代，因为uuid每次运行都会变，无法做到同一文件对应同一ID
                # 为每个父文档分配确定性的唯一ID（基于数据根目录的相对路径）
                # 为每个文档生成一个 “确定性的唯一 ID”（即只要文件路径不变，ID 就固定不变）
                try:
    ...
    def _enhance_metadata(self, doc: Document):
        ...
        # 对于 Linux/macOS 路径"data/recipes/番茄炒蛋.md"，parts会返回 ('data', 'recipes', '番茄炒蛋.md')。
        ...
        # Path 对象的 stem 属性专门用于获取文件名中 “去掉后缀的部分”（可以理解为 “纯文件名”）。
        # 例如，对于路径 "data/recipes/番茄炒蛋.md"，stem 会返回 "番茄炒蛋"。
        # 分析难度等级
    ...
    def chunk_documents(self) -> List[Document]:
        ...
        for i, chunk in enumerate(chunks):
            ...
            # 所有子块的索引，
            # 假设 “番茄炒蛋” 的 3 个子块和 “红烧肉” 的 2 个子块汇总后，
            # chunks 列表顺序为 [蛋块0, 蛋块1, 蛋块2, 肉块0, 肉块1]，
            # 则它们的 batch_index 分别是 0、1、2、3、4。
